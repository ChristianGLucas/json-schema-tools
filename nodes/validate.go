...
func Validate(ctx context.Context, ax axiom.Context, input *gen.ValidateRequest) (*gen.ValidateResponse, error) {
	if len(input.Schema) > maxSchemaBytes {
		return &gen.ValidateResponse{Error: fmt.Sprintf("schema exceeds size limit of %d bytes", maxSchemaBytes)}, nil
	}
	if len(input.Instance) > maxInstanceBytes {
		return &gen.ValidateResponse{Error: fmt.Sprintf("instance exceeds size limit of %d bytes", maxInstanceBytes)}, nil
	}
